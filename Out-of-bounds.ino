...
void loop() {
  ...
  if(line[3]>=160||line[2]>=255||line[1]>=210){
    if(dir<50&&dir>300) dir=180;
    else dir=90;
    line_detected=1;
  }
  if(line[9]>=125||line[8]>=170||line[7]>=150){
    if(dir<50&&dir>300) dir=180;
    else dir=270;
    line_detected=1;
  }
  if((line[9]>=125||line[8]>=170||line[7]>=140)&&(line[3]>=145||line[2]>=250||line[1]>=210)){
    if(dir<90&&dir>270) dir=180;
    else dir=0;
    line_detected=1;
  }
  if(line[6]>=210||line[5]>=225||line[4]>=260){
    dir=180;
    line_detected=1;
  }
  ...
  r=abs(IR.Strength-130);
  x=cos(ir[IR.Direction]*M_PI/180);
  dir= IR.Direction>5? acos((r*x-h)/r)*(180/M_PI): 360-(acos((r*x-h)/r)*(180/M_PI));
  if(IR.Direction>7||IR.Direction<3) dir=ir[IR.Direction];
  if(IR.Direction==5) dir=0;
  if(IR.Direction==0){
    if(last_pos<5) dir=140;
    else dir=220;
  }
